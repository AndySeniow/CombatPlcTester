 ...
 * - **Command Pattern:**
 *   - Encapsulates modifications (e.g., updating or deleting elements) into commands, enabling:
 *
 * Workflow:
 ...
 * 5. Updates (e.g., text changes, positional adjustments) or deletions are encapsulated into a command (e.g., `UpdateCommand` or `DeleteCommand`).
 * 6. The command is passed to a controller (e.g., `TestController`) for execution, modifying the model.
 *
 * @author Seniow Andy
 * @version 1.0
 * @since 13/01/2025
...
public class UpdateDeleteHandler implements StateHandler{
    ...
    */
    ...
    /**
    * Moves the selected state to a new position and updates the positions of any connected transitions to maintain consistency.
    *
    * Workflow:
    ...
    */
    ...
    /**
    * Moves the control points of the selected transition based on the new mouse position, allowing for precise adjustment of the transition path.
    *
    * Workflow:
    * - Creates a point with the new coordinates.
